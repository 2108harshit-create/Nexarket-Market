...
import {
  User, Product, CartItem, Order, Review, Message, Notification, WishlistItem, SEED_USERS,
...
useEffect(() => {
  const savedUsers = localStorage.getItem('nexarket_users');
  if (savedUsers) {
    setUsers(JSON.parse(savedUsers) as User[]);
  } else {
    setUsers(SEED_USERS);
    localStorage.setItem('nexarket_users', JSON.stringify(SEED_USERS));
  }
}, []);
  ...
  const login = useCallback((email: string, password: string) => {
    const user = users.find(u => u.email === email && password === "demo123") || 
                 (email === "buyer@demo.com" ? DEMO_BUYER : email === "seller@demo.com" ? DEMO_SELLER : null);
    if (user) {
      setCurrentUser(user);
      return { success: true };
    }
  ...
  const register = useCallback((name: string, email: string, password: string, role: "buyer" | "seller") => {
    ...
    setUsers(prev => {
      ...
      localStorage.setItem('nexarket_users', JSON.stringify(updated));
      return updated;
    });
  ...
  // Save currentUser to localStorage on change
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem('nexarket_current_user', JSON.stringify(currentUser));
    } else {
      localStorage.removeItem('nexarket_current_user');
    }
  }, [currentUser]);
